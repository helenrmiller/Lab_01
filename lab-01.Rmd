...
visitors %>%
  filter(setting=='O', region == 'Yorkshire and the Humber', admission=='Members', n_2022>100000 )
```
...
```
see bottom line of table, data class is shown.
...
```
```{r}
#visitors %>% filter(5E4<=n_2022<=1E5) %>%
  #count()
...
**`NA` stands for not available, because some of the values from the 2021 dataset are missing and calculations cannot be performed on missing values.**
...
**If mean > median, then the data is skewed right. This means there are probably a few highly visited sights, while most are visited far less frequently.**
...
visitors_with_nation %>%
  count(nation)
```
The IQR for England and Scotland are probably fairly reliable, but the values for Wales and Northern Ireland cannot be determined accurately.
...
```{r Ex_m}
total <- visitors_with_nation %>%
  count(nation)

free <- visitors_with_nation %>%
  group_by(nation) %>%
    summarise(prop_free = sum(admission == "Free") / n_distinct(attraction))
free


#at_p_nation <- count(visitors_with_nation, nation) %>%
 # group_by(nation)
#free_p_nation <- count(visitors_with_nation, nation, admission == 'Free') %>%
#  group_by(nation)
#at_p_nation[n] / free_p_nation[n]
  
  
  
#mutate(visitors_with_nation
 #      prop_free_admin = case when(
...
```
HAVING A MAJOR STRUGGLE HERE, WILL REVIEW LATER
